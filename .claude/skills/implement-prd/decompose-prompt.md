...
6. **Validation gates.** Default: `["typecheck", "lint:touched"]`. Add `test:targeted` only if the PRD lists test files for the task. Never add `build` per-task (too slow); reserve `build` for end-of-loop.
   - For tasks touching **only** `*.test.ts(x)`, `*.stories.tsx`, or `*.md`/`*.mdx`: drop `typecheck` (no production code paths). Use `["lint:touched"]` (+ `test:targeted` if applicable). Saves ~5–10s per such task.
   - For pure-deletion tasks (no new files, no modifications, only `rm`): `validation` may be `[]`. Lint has nothing to check; the end-of-loop `build` step catches any orphan-import damage.
...
13. **Trivial-task batching (cost floor).** Each cold sub-agent costs ~30–60s of orientation plus the 60s `ScheduleWakeup` floor. If 2+ tasks each touch ≤1 file, have ≤1 acceptance bullet, and don't add a `validation` gate beyond the default, **merge them** when their combined `files` ≤5 — even when they touch unrelated areas (e.g. a 1-line FAQ copy edit + a 1-file coverflow filter edit + a script-default flip). The orientation tax dominates real work for trivial tasks; batch ruthlessly. Don't merge if the tasks have meaningfully different `validation` arrays (e.g. one needs `test:targeted` and the other doesn't).

14. **Test-symbol coupling (avoid cascade halts).** When a task deletes or renames an exported symbol, the same task — or its **immediate prior** in `depends_on` — **must** include in its `files` list every test file referencing that symbol. Compute by grepping `__tests__/` (and any other test directories) for the symbol name. Skipping this causes the loop sub-agent to self-halt at delete time when its hard-rule grep finds remaining imports. Audit shows this added ~3 min and one wasted iteration on the prior run.

15. **URL-contract sweep (catch state-mirror drift).** When a task adds, removes, or renames a `nuqs` query param (or any other shared URL-state contract), grep the source tree for the param key as a **string literal** (e.g. `"month"`, `"q"`, `"level"`). Add **every consumer** found to that task's `files` list — not just the canonical hook. Common consumers besides the hook: hero/stats components that mirror filter state with their own `useQueryState` calls, ActiveFilterStrip-style banners, sticky CTAs that show "X filters active". TypeScript and ESLint cannot detect a missed consumer; the bug only surfaces in browser smoke. Audit shows this caused one shipped bug in the prior run (`scholarship-hero-stats.tsx anyFilterActive` missing the new `month` param).

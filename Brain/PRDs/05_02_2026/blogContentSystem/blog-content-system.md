...
`src/data/blog-authors.ts` exports a typed `Record<AuthorKey, BlogAuthor>` keyed by short string (`Cathy Dumenu`). Frontmatter references the key only (`author: Cathy Dumenu`). The existing six authors from the dummy file are preserved verbatim — names, roles, avatar paths.
...
- Each post is assigned an author from the existing six (Cathy Dumenu for essay/personal-statement-style, James for mistakes/recommendations, Amara for first-gen) and a `coverImage` pointing at one of the existing photos in `public/scholarships/` (each post gets a distinct photo).
...
2. Should the four migrated posts share an author, or be distributed across the existing six? **Proposal:** distribute by topic fit (Cathy Dumenu: essay/statement; James: mistakes/recommendations; Amara: first-gen success).
